...
export class PanelLayoutManager implements AppModule {
      ...
      <div class="header">
        <div class="header-left">
          <div class="variant-switcher">${(() => {
            ...
            return `
            <a href="${vHref('finance', 'https://intel.tesserect.com')}"
            ...
            <a href="${vHref('tesserect', 'https://intel.tesserect.com')}"
               class="variant-option ${SITE_VARIANT === 'tesserect' ? 'active' : ''}"
               data-variant="tesserect"
               ${vTarget('tesserect')}
               title="${t('header.tesserect')}${SITE_VARIANT === 'tesserect' ? ` ${t('common.currentVariant')}` : ''}">
              <span class="variant-icon">📦</span>
              <span class="variant-label">${t('header.tesserect')}</span>
            </a>`;
          })()}</div>
          <span class="logo">${SITE_VARIANT === 'tesserect' ? 'TESSERECT' : 'MONITOR'}</span><span class="version">v${__APP_VERSION__}</span>${BETA_MODE ? '<span class="beta-badge">BETA</span>' : ''}
          ${SITE_VARIANT === 'tesserect' ? `
          <a href="https://www.tesserect.co.za" target="_blank" rel="noopener" class="credit-link" title="Tesserect Investments">
            <span class="credit-text">Tesserect</span>
          </a>` : `
        ...
        <div class="map-section" id="mapSection">
          <div class="panel-header">
            <div class="panel-header-left">
              <span class="panel-title">${SITE_VARIANT === 'tesserect' ? 'Trade Map' : t('panels.map')}</span>
            </div>
  ...
  private createPanels(): void {
    ...
    if (savedOrder.length > 0) {
      ...
      const insertIdx = valid.indexOf('live-news') + 1 || valid.indexOf('markets') + 1 || 0;
